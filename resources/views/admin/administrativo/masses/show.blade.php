@extends('admin.layout')

@section('title', 'Missa - ' . $mass->day_name . ' às ' . $mass->time->format('H:i'))

@section('content')
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-start mb-3">
        <div>
            <h1 class="h4 mb-1">{{ $mass->day_name }} - {{ $mass->time->format('H:i') }}</h1>
            <div class="text-muted small d-flex align-items-center gap-2 flex-wrap">
                <span>{{ $mass->location }}</span>
                <span>•</span>
                <span class="badge {{ $mass->is_active ? 'bg-success' : 'bg-danger' }}">{{ $mass->is_active ? 'Ativo' : 'Inativo' }}</span>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('admin.administrativo.masses.edit', $mass) }}" class="btn btn-primary"><i class="bi bi-pencil me-2"></i>Editar</a>
            <a href="{{ route('admin.administrativo.masses.index') }}" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-2"></i>Voltar</a>
        </div>
    </div>

    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <i class="bi bi-check-circle-fill me-2"></i>{{ session('success') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    @endif

    <div class="row g-4">
        <div class="col-lg-8">
            <div class="card shadow-sm">
                <div class="card-body">
                    <div class="mb-3">
                        <h6 class="mb-2">Detalhes</h6>
                        <div class="small text-muted">
                            <div><span class="fw-semibold">Dia:</span> {{ $mass->day_name }}</div>
                            <div><span class="fw-semibold">Hora:</span> {{ $mass->time ? $mass->time->format('H:i') : '' }}</div>
                            <div><span class="fw-semibold">Local:</span> {{ $mass->location }}</div>
                        </div>
                    </div>
                    @if($mass->description)
                        <div>
                            <h6 class="mb-2">Descrição</h6>
                            <div class="text-muted">{!! nl2br(e($mass->description)) !!}</div>
                        </div>
                    @endif
                </div>
            </div>
        </div>
        <div class="col-lg-4">
            <div class="card shadow-sm mb-3">
                <div class="card-body">
                    <h6 class="card-title mb-3">Ações</h6>
                    <div class="d-grid gap-2">
                        <a href="{{ route('admin.administrativo.masses.edit', $mass) }}" class="btn btn-primary"><i class="bi bi-pencil me-2"></i>Editar</a>
                        <form method="POST" action="{{ route('admin.administrativo.masses.destroy', $mass) }}" onsubmit="return confirm('Tem certeza que deseja excluir esta missa?')" class="w-100">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger w-100"><i class="bi bi-trash me-2"></i>Excluir</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="card shadow-sm">
                <div class="card-body">
                    <h6 class="card-title mb-3">Navegação</h6>
                    <div class="d-grid gap-2">
                        <a href="{{ route('admin.administrativo.masses.index') }}" class="btn btn-link text-start">← Todos os Horários</a>
                        <a href="{{ route('admin.administrativo.dashboard') }}" class="btn btn-link text-start">🏠 Dashboard</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
