...
<div class="container-fluid px-4 py-4">
    <div class="card border-0 shadow-sm mb-4" style="background: linear-gradient(135deg, #8B1538 0%, #6E1530 50%, #8B1538 100%); border-radius: 15px;">
        <div class="card-body text-white py-4 px-4">
            <h1 class="h2 fw-bold mb-2" style="color: #FFFFFF;"><i class="bi bi-file-earmark-pdf" aria-hidden="true"></i> Escalas PDF - {{ $group->name }}</h1>
            <p class="mb-0" style="color: #FFD66B;">Gerencie as escalas em PDF do seu grupo</p>
        </div>
    </div>

    <div class="card border-0 shadow-sm mb-4">
        <div class="card-header bg-white py-3"><h5 class="mb-0 fw-bold"><i class="bi bi-cloud-upload" aria-hidden="true"></i> Enviar Nova Escala</h5></div>
        <div class="card-body">
            <form action="{{ route('admin.coordenador.scales.upload') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <label for="title" class="form-label fw-semibold"><i class="bi bi-card-heading" aria-hidden="true"></i> Título da Escala</label>
                        <input type="text" id="title" name="title" class="form-control" required placeholder="Ex: Escala de Coroinhas - Dezembro 2025">
                    </div>
                    <div class="col-md-6">
                        <label for="file" class="form-label fw-semibold"><i class="bi bi-file-pdf" aria-hidden="true"></i> Arquivo PDF</label>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <label for="valid_from" class="form-label fw-semibold"><i class="bi bi-calendar-check" aria-hidden="true"></i> Válido a partir de</label>
                        <input type="date" id="valid_from" name="valid_from" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label for="valid_until" class="form-label fw-semibold"><i class="bi bi-calendar-x" aria-hidden="true"></i> Válido até</label>
                        <input type="date" id="valid_until" name="valid_until" class="form-control">
                    </div>
                </div>
                <div class="mb-3">
                    <label for="description" class="form-label fw-semibold"><i class="bi bi-text-paragraph" aria-hidden="true"></i> Descrição (Opcional)</label>
                ...
                <div class="text-end"><button type="submit" class="btn btn-primary btn-lg"><i class="bi bi-cloud-upload me-2" aria-hidden="true"></i>Enviar Escala</button></div>
            </form>
        </div>
    </div>

    <div class="card border-0 shadow-sm">
        <div class="card-header bg-white py-3"><h5 class="mb-0 fw-bold"><i class="bi bi-file-earmark-text" aria-hidden="true"></i> Escalas Enviadas</h5></div>
        @if($scales->count() > 0)
                        ...
                        <tbody>
                            @foreach($scales as $scale)
                                <tr>
                                    ...
                                    <td><div class="d-flex align-items-center gap-2"><i class="bi bi-file-pdf-fill text-danger" aria-hidden="true" style="font-size: 1.5rem;"></i><div><div>{{ $scale->original_filename }}</div><small class="text-muted">({{ $scale->file_size_human }})</small></div></div></td>
                                    ...
                                    <td class="text-center">@if($scale->isValid())<span class="badge bg-success"><i class="bi bi-check-circle" aria-hidden="true"></i> Ativo</span>@else<span class="badge bg-warning"><i class="bi bi-pause-circle" aria-hidden="true"></i> Inativo</span>@endif</td>
                                    <td><div>{{ $scale->created_at->format('d/m/Y H:i') }}</div><small class="text-muted">por {{ $scale->uploader->name }}</small></td>
                                    <td class="text-center"><div class="d-flex gap-2 justify-content-center"><button type="button" class="btn btn-sm btn-outline-success" onclick="viewPDF('{{ asset('storage/' . $scale->file_path) }}', '{{ addslashes($scale->title) }}')" title="Visualizar PDF"><i class="bi bi-eye me-1" aria-hidden="true"></i>Ver</button><a href="{{ route('admin.coordenador.scales.download', $scale) }}" class="btn btn-sm btn-outline-primary" title="Baixar PDF"><i class="bi bi-download me-1" aria-hidden="true"></i>Baixar</a><form action="{{ route('admin.coordenador.scales.destroy', $scale) }}" method="POST" class="d-inline" onsubmit="return confirm('Tem certeza que deseja remover esta escala?')">@csrf @method('DELETE')<button type="submit" class="btn btn-sm btn-outline-danger" title="Remover"><i class="bi bi-trash me-1" aria-hidden="true"></i>Remover</button></form></div></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        @else
            <div class="card-body text-center py-5">
                <div class="mb-4"><i class="bi bi-file-earmark-pdf" aria-hidden="true" style="font-size: 4rem; color: #dee2e6;"></i></div>
